...
class DPT(BaseModel):
    def __init__(
        ...
        **kwargs
        ...
        # For the Swin, Swin 2, LeViT and Next-ViT Transformers, the
        # hierarchical architectures prevent setting the hooks freely.
        # Instead, the hooks have to be chosen according to the ranges
        ...
        self.pretrained, self.scratch = _make_encoder(
            backbone,
            features,
            False,  # Set to true to train from scratch
            groups=1,
            expand=False,
            exportable=False,
            hooks=hooks,
...
class DPTDepthModel(DPT):
    ...
    def forward(self, x):
        ...
        if self.return_features:
            ...
            return result[0].squeeze(dim=1), result[1]
        else:
            return result.squeeze(dim=1)


class DPTSegmentationModel(DPT):
    def __init__(self, num_classes=3, path=None, **kwargs):
        features = kwargs["features"] if "features" in kwargs else 256
